...
export default function ProjectPageClient({ id }: { id: string }) {
  ...
  return (
    <main className="relative min-h-screen bg-black text-white px-8 md:px-10 py-10 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_50%_-20%,rgba(230,162,60,0.06),transparent)] z-0" />
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              {navSections.map((section, index) => (
                <button
                  ...
                  className={`glass-pill ${navColors[index % navColors.length]} text-xs text-white/90 px-3 py-1.5 rounded-full transition-colors hover:text-white`}
                >
                  {section.label}
                </button>
              ))}
